<template>
  <div>
    <div class="detail" v-for="(fangdong,index) in FangDong">
      <router-link to="/goodDetail"><img :src="fangdong.Images" alt=""></router-link>
      <p>￥:{{fangdong.PriceFormart}}</p>
      <div class="content">
        <span>自营</span>
        <router-link to="/goodDetail">{{fangdong.ProductName}}</router-link>
      </div>
      <div class="salesVolume">
        已售<span>{{fangdong.salesVolume}}</span>件
      </div>
      <div class="all">
      ...
      <div class="joinCar">
        <span @click="addCart(fangdong,index)">加入购物车</span>
      </div>
    </div>
...
<script>
  ...
  import FangDongData from "../data/FangDongData"
  ...
  export default {
    ...
    mounted:function () {
      this.FangDong = FangDongData.Data.DataList;
      console.log(FangDongData.Data.DataList)
    },
    methods: {
      add(index) {
