<template>
  <div>
    <div class="detail" v-for="(huohua,index) in HuoHua">
      <router-link to="/goodDetail"><img :src="huohua.Images" alt=""></router-link>
      <p>￥:{{huohua.PriceFormart}}</p>
      <div class="content">
        <span>自营</span>
        <router-link to="/goodDetail">{{huohua.ProductName}}</router-link>
      </div>
      <div class="salesVolume">
        已售<span>{{huohua.salesVolume}}</span>件
      </div>
      <div class="all">
      ...
      <div class="joinCar">
        <span @click="addCart(huohua,index)">加入购物车</span>
      </div>
    </div>
...
<script>
  ...
  import HuoHuaData from "../data/HuoHuaData"
  ...
  export default {
    ...
    mounted:function () {
      this.HuoHua = HuoHuaData.Data.DataList;
      console.log(HuoHuaData.Data.DataList)
    },
    methods:{
      add(index) {
