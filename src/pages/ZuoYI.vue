<template>
  <div>
    <div class="detail" v-for="(zuoyi,index) in ZuoYi">
      <router-link to="/goodDetail"><img :src="zuoyi.Images" alt=""></router-link>
      <p>￥:{{zuoyi.PriceFormart}}</p>
      <div class="content">
        <span>自营</span>
        <router-link to="/goodDetail">{{zuoyi.ProductName}}</router-link>
      </div>
      <div class="salesVolume">
        已售<span>{{zuoyi.salesVolume}}</span>件
      </div>
      <div class="all">
      ...
      <div class="joinCar">
        <span @click="addCart(zuoyi,index)">加入购物车</span>
      </div>
    </div>
...
<script>
  ...
  import ZuoYiData from "../data/ZuoYiData"
  ...
  export default {
    ...
    mounted:function () {
      this.ZuoYi = ZuoYiData.Data.DataList;
      // console.log(ZuoYiData.Data.DataList[0].Images)
    },
    methods:{
      add(index) {
