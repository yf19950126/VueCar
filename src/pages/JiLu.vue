<template>
  <div>
    <div class="detail" v-for="(jilu,index) in JiLu">
      <router-link to="/goodDetail"><img :src="jilu.Images" alt=""></router-link>
      <p>￥:{{jilu.PriceFormart}}</p>
      <div class="content">
        <span>自营</span>
        <router-link to="/goodDetail">{{jilu.ProductName}}</router-link>
      </div>
      <div class="salesVolume">
        已售<span>{{jilu.salesVolume}}</span>件
      </div>
      <div class="all">
      ...
      <div class="joinCar">
        <span @click="addCart(jilu,index)">加入购物车</span>
      </div>
    </div>
...
<script>
  ...
  import JiLuData from "../data/JiLuData"
  ...
  export default {
    ...
    mounted:function () {
      this.JiLu = JiLuData.Data.DataList;
      console.log(JiLuData.Data.DataList)
    },
    methods:{
      add(index) {
