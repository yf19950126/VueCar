<template>
  <div>
    <div class="detail" v-for="(boli,index) in BoLi">
      <router-link to="/goodDetail"><img :src="boli.Images" alt=""></router-link>
      <p>￥:{{boli.PriceFormart}}</p>
      <div class="content">
        <span>自营</span>
        <router-link to="/goodDetail">{{boli.ProductName}}</router-link>
      </div>
      <div class="salesVolume">
        已售<span>{{boli.salesVolume}}</span>件
      </div>
      <div class="all">
      ...
      <div class="joinCar">
        <span @click="addCart(boli,index)">加入购物车</span>
      </div>
    </div>
...
<script>
  import BoLiData from "../data/BoLiData"
  ...
  export default {
    ...
    mounted:function () {
      this.BoLi = BoLiData.Data.DataList;
      console.log(BoLiData.Data.DataList[0].Images)
    },
    methods:{
      add(index) {
      ...
      remove(index) {
        let num =parseInt(document.getElementsByClassName("num-middle")[index].innerHTML);
        if(num<=0){
          num = 0;
          document.getElementsByClassName("num-middle")[index].innerHTML=num;
        }else{
