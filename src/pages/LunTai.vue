<template>
  <div>
    <div class="detail" v-for="(luntai,index) in LunTai">
      <router-link to="/goodDetail"><img :src="luntai.Images" alt=""></router-link>
      <p>￥:{{luntai.PriceFormart}}</p>
      <div class="content">
        <span>自营</span>
        <router-link to="/goodDetail">{{luntai.ProductName}}</router-link>
      </div>
      <div class="salesVolume">
        已售<span>{{luntai.salesVolume}}</span>件
      </div>
      <div class="all">
      ...
      <div class="joinCar">
        <span @click="addCart(luntai,index)">加入购物车</span>
      </div>
    </div>
...
<script>
  ...
  import LunTaiData from "../data/LunTaiData"
  import MyFooter from "../components/MyFooter"
    export default {
      ...
      mounted:function () {
        this.LunTai = LunTaiData.Data.DataList;
        //console.log(LunTaiData.Data.DataList)
      },
      methods:{
        add(index) {
