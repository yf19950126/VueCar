<template>
  <div>
    <div class="detail" v-for="(jiyou,index) in JiYou">
      <router-link to="/goodDetail"><img :src="jiyou.Images" alt="">
      </router-link>
      <p>￥:{{jiyou.PriceFormart}}</p>
      <div class="content">
        <span>自营</span>
        <router-link to="/goodDetail">{{jiyou.ProductName}}</router-link>
      </div>
      <div class="salesVolume">
        已售<span>{{jiyou.salesVolume}}</span>件
      </div>
      <div class="all">
      ...
      <div class="joinCar">
        <span @click="addCart(jiyou,index)">加入购物车</span>
      </div>
    </div>
...
<script>
  ...
  import JiYouData from "../data/JiYouData"
  ...
  export default {
    ...
    mounted:function () {
      this.JiYou = JiYouData.Data.DataList;
      console.log(JiYouData.Data.DataList)
    },
    methods:{
      add(index) {
